...
def spawn_chapter_links_spider(website_name, novel_link):
    os.system(
        "gnome-terminal -- "
        + f"bash -c 'source .re_venv/bin/activate; python3 manage.py spawn_chapter_link_pages_spider '{website_name}' '{novel_link}'; exit;'"
    )
...
def spawn_chapter_pages_spider(website_name, novel_link):
    os.system(
        "gnome-terminal -- "
        + f"bash -c 'source .re_venv/bin/activate; python3 manage.py spawn_chapter_pages_spider '{website_name}' '{novel_link}'; exit;'"
    )
